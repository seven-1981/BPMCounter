...
TEST_F(TestBuffer, BuferInitWithZeroNotPossible)
{
    Buffer<short unsigned int> zeroBuffer;

    zeroBuffer.init(0);

    ASSERT_DEATH(zeroBuffer[0], "");
}

TEST_F(TestBuffer, InitializerListWorksProperly)
{
	Buffer<short signed int> initBuffer = { 0, 1, -2, 3, 4 };

	ASSERT_EQ(initBuffer.get_size(), 5);
	ASSERT_EQ(initBuffer[2], -2);
}
